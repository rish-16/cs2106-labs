...
void init_barrier(int numproc) {
    ...
    barrier_object->count = (int *)malloc(sizeof(int));
    *(barrier_object->count) = 0;

    barrier_object->sem1 = (sem_t *)malloc(sizeof(sem_t));
...
void reach_barrier() {
    ...
    *(barrier_object->count) = *(barrier_object->count) + 1; // increment count when new processs reaches barrier
